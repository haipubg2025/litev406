...
import { X, Wrench, Sparkles, ImagePlus, UserPlus, Loader2, Check, ExternalLink, ChevronDown, ChevronUp } from "lucide-react";
import { useStore } from "../store/useStore";
import { aiService } from "../services/aiService";
import { Type } from "@google/genai";
import { toast } from "sonner";
import ReactMarkdown from "react-markdown";
import { escapeLiteralNewlinesInJson, repairTruncatedJson } from "../utils/jsonRepair";

const ExpandableText = ({ label, text, theme }: { label: string, text: string, theme: any }) => {
  ...
  return (
    <div className="flex flex-col space-y-1 mb-2">
      <div className="flex items-center justify-between">
        <strong className={theme.group === 'Dark' ? 'text-cyan-400' : 'text-cyan-700'}>{label}:</strong>
        <div className="flex items-center gap-1">
          <button onClick={() => setIsExpanded(!isExpanded)} className={`px-2 py-0.5 text-[10px] uppercase font-bold rounded ${theme.group === 'Dark' ? 'bg-blue-500/20 text-blue-300 hover:bg-blue-500/30' : 'bg-blue-500/10 text-blue-600 hover:bg-blue-500/20'}`}>
...
export default function NPCBuilderModal({ onClose }: NPCBuilderModalProps) {
  const { theme, gameData, setGameData, npcBuilder, setNpcBuilder } = useStore();
      ...
      properties: {
        newNPCs: {
          ...
          items: {
              ...
              relationships: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    termsOfAddress: { type: Type.ARRAY, items: { type: Type.STRING } },
                    description: { type: Type.STRING }
                  }
                }
              },
            ...
            required: ["name", "fullName", "titles", "occupation", "role", "background", "gender", "age", "dob", "rank", "height", "weight", "measurements", "appearance", "appearanceLite", "personality", "personalityCore", "philosophy", "goal", "distinguishingFeatures", "innerSecret", "relationships", "loveViews", "experience", "nsfwPersonality", "nsfwReactions", "literaryDescription", "preferences"]
          }
        }
      },
      required: ["newNPCs"]
...
2. TUYỆT ĐỐI CHỐNG CẢM XÚC HÓA GƯƠNG MẶT: BẮT BUỘC CẤM 100% miêu tả nụ cười (cười mỉm, cười lạnh, nụ cười mãn nguyện...), khóe môi (cong lên, nhếch môi, nhéo môi...), ánh mắt (sắc lẹm, trầm xuống...) và chân mày (nhíu mày, cau mày...). Phải loại bỏ hoàn toàn biểu cảm khuôn mặt cơ học. Yêu cầu AI phải chuyển hướng tập trung hoàn toàn vào việc miêu tả hành động tay chân, lời thoại trực tiếp và nội tâm thay vì ngắm nghía cơ mặt của nhân vật.
3. NGHIÊM CẤM 100% TỪ VỰNG: "nhếch mép", "tà mị", "nhếch môi". KHÔNG BAO GIỜ SỬ DỤNG!
4. TUYỆT ĐỐI TÔN TRỌNG ĐẠO ĐỨC & KHÔNG HẮC HÓA / DÂM HÓA TÙY TIỆN: Khi tạo ra các npc ở mọi chức năng, AI phải tuyệt đối tuân thủ theo định hướng và gợi ý của người chơi, tôn trọng các giá trị đạo đức và giới hạn SFW/NSFW. TUYỆT ĐỐI không được phép tha hóa, hắc hóa, tô xấu, hoặc "dâm hóa" (gán ghép bản tính lăng loàn, lẳng lơ, dâm đãng vô cớ) cho NPC nếu người chơi không hề gợi ý hay yêu cầu điều đó. Tôn trọng bản ngã gốc và sự trong sáng của nhân vật nếu không có chỉ thị ngược lại.
5. KIỂM SOÁT TỪ NGỮ THEO BỐI CẢNH (ĐẶC BIỆT LÀ TỪ HÁN VIỆT): Từ Hán Việt bao gồm các từ hành chính, xã hội thông dụng và cả những từ đặc thù cổ trang/Tiên Hiệp. Tùy thuộc vào bối cảnh thế giới (Fantasy, Isekai, Hiện đại hay Cổ trang) mà AI phải chọn lọc từ ngữ cho chuẩn xác. TUYỆT ĐỐI KHÔNG dùng từ Hán Việt mang sắc thái Kiếm Hiệp/Tiên Hiệp/Cổ trang Phương Đông cho các bối cảnh phương Tây, Isekai, Sci-Fi. Ở các bối cảnh phi Phương Đông, chỉ dùng từ thuần Việt hoặc từ Hán Việt mang tính phổ quát (hành chính, khoa học, xã hội) để tránh cảm giác lạc quẻ, kì cục.
...
- relationships: Danh sách các tương quan (Mảng object [{name, relation, status, impression, termsOfAddress, description}]). (NGUYÊN TẮC: TUYỆT ĐỐI KHÔNG TẠO QUAN HỆ VỚI KHÁCH THỂ CHƯA XUẤT HIỆN. Để trống mảng nếu không quen ai. Tên npc phải ghi đầy đủ họ tên. ĐẶC BIỆT: Khi tạo mối quan hệ, BẮT BUỘC phải điền đầy đủ nội dung cho "impression" (Ấn tượng và suy nghĩ chi tiết về đối phương) và "termsOfAddress" (Mảng các cách xưng hô thường dùng với đối phương, VD [" huynh", " đại ca"]). NGHIÊM CẤM lười biếng bỏ trống 2 mục này!).
...
Tôn trọng luật SHOW DON'T TELL.`;
    ...
    const existingNames = (gameData?.npcs || []).map((n: any) => n.fullName || n.name).join(", ");
  ...
  "newNPCs": [
    {
      ...
      "relationships": [{ "name": "Họ và Tên", "relation": "Quan hệ", "status": "Trạng thái", "impression": "Ấn tượng và suy nghĩ chi tiết (BẮT BUỘC ĐIỀN ĐẦY ĐỦ)", "termsOfAddress": ["Cách xưng hô 1 (BẮT BUỘC)", "Cách xưng hô 2"], "description": "Mô tả" }],
...
\`\`\`\n\n`;
    finalPrompt += `Hãy phân tích hình ảnh (nếu có) và yêu cầu sau để tạo ra một hoặc nhiều NPC mới cho Game, ĐẢM BẢO KHÔNG TRÙNG LẶP VỚI CÁC NHÂN VẬT ĐÃ CÓ VÀ PHÙ HỢP LOGIC THẾ GIỚI MÀ NGƯỜI CHƠI ĐANG CHƠI (${existingNames}).\n\n`;
    finalPrompt += `YÊU CẦU: ${prompt}\n`;

    try {
      let fullJsonStr = "";
      let thinkingText = "";
      const stream = aiService.generateStreamingContent(finalPrompt, undefined, sysInstruction, images);
      ...
      for await (const chunk of stream) {
        ...
        const displayStr = (thinkingText ? thinkingText + "\n\n" : "") + fullJsonStr;
        setStreamText(displayStr);
      }

      let cleanedJsonStr = fullJsonStr;
      
      // Extract everything between ```json and ```
      const jsonBlockRegex = /```json\s*([\s\S]*?)\s*```/i;
      const match = cleanedJsonStr.match(jsonBlockRegex);
      
      if (match && match[1]) {
        cleanedJsonStr = match[1];
      } else {
        // Fallback: Try to extract an unclosed ```json block
        const unclosedJsonBlockRegex = /```json\s*([\s\S]*)/i;
        const unclosedMatch = cleanedJsonStr.match(unclosedJsonBlockRegex);
        if (unclosedMatch && unclosedMatch[1]) {
          cleanedJsonStr = unclosedMatch[1];
        } else {
          // Remove any closed thinking process before trying to find braces
          cleanedJsonStr = cleanedJsonStr.replace(/<THINKING_PROCESS>[\s\S]*?<\/THINKING_PROCESS>/gi, "");
          
          // Try to extract everything between the first { and the last }
          const firstBrace = cleanedJsonStr.indexOf("{");
          const lastBrace = cleanedJsonStr.lastIndexOf("}");
          if (firstBrace !== -1 && lastBrace !== -1 && lastBrace >= firstBrace) {
            cleanedJsonStr = cleanedJsonStr.substring(firstBrace, lastBrace + 1);
          }
        }
      }

      cleanedJsonStr = escapeLiteralNewlinesInJson(cleanedJsonStr);
      cleanedJsonStr = repairTruncatedJson(cleanedJsonStr);
      
      console.log("Attempting to parse AI response:", cleanedJsonStr);
      ...
      try {
        ...
        if (Array.isArray(parsedData)) {
            ...
            const arrayKeys = Object.keys(parsedData).filter(k => Array.isArray(parsedData[k]));
        ...
        console.error("Parse error string:", cleanedJsonStr);
      }
    } catch (error: any) {
  ...
  return (
    ...
    >
      ...
      >
        <div className={`p-4 md:p-6 flex items-center justify-between border-b shrink-0 ${theme.group === "Dark" ? "border-white/10 bg-white/5" : "border-black/10 bg-black/5"}`}>
          <div className="flex items-center gap-3">
            <div className={`p-1.5 rounded-lg border ${theme.group === "Dark" ? "bg-white/10 border-white/10" : "bg-black/5 border-black/10"} shadow-inner`}>
               <Wrench size={18} className={theme.textPrimary} />
            </div>
            <div className="flex items-center gap-3">
              <h2 className="text-sm md:text-base font-bold tracking-widest uppercase">Nơi Tạo NPC Mới</h2>
              <div className="flex items-center gap-1">
                <button 
                  onClick={() => modalScrollRef.current?.scrollTo({ top: 0, behavior: 'auto' })}
                ...
                <button 
                  onClick={() => modalScrollRef.current?.scrollTo({ top: modalScrollRef.current.scrollHeight, behavior: 'auto' })}
                  className={`p-1 rounded shadow-sm ${theme.group === 'Dark' ? 'bg-white/10 hover:bg-white/20 text-white' : 'bg-black/10 hover:bg-black/20 text-black'} transition-all`}
        ...
        <div ref={modalScrollRef} className="flex-1 p-4 md:p-6 overflow-y-auto w-full max-w-6xl mx-auto flex flex-col gap-6 custom-scrollbar pb-24">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="w-full md:w-1/3 flex flex-col">
                ...
                >
                  ...
                  <span className="text-sm opacity-60 font-medium tracking-wide">Tải Lên (Tối đa 3)</span>
                </div>
                <input
                  type="file"
                  multiple
            ...
            {images.length > 0 && (
              <div className="flex flex-wrap gap-4 mt-2">
                {images.map((img, idx) => (
                  <div key={idx} className="relative group rounded-xl overflow-hidden shadow-lg border border-white/10 w-24 h-24 md:w-32 md:h-32">
                    <img src={img} alt="Tải lên" className="w-full h-full object-cover transition-transform group-hover:scale-105" />
                    <button
            ...
            >
              ...
              {isGenerating ? "Hệ thống đang cấu trúc nhân vật..." : "Khởi Tạo NPC"}
            </button>
                ...
                >
                  <div className="flex items-center justify-between px-4 py-2 border-b border-white/10 bg-white/5">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[10px] uppercase font-bold tracking-widest text-white/70">AI Core Terminal</span>
                      {isGenerating && (
                    ...
                    >
                      {isStreamExpanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                    </button>
                  </div>
                  <div className={`p-4 font-mono text-xs overflow-y-auto leading-relaxed whitespace-pre-wrap transition-all ${isStreamExpanded ? 'h-96' : 'h-32'} text-green-500/80`}>
              ...
              >
                ...
                {isGenerating && !generatedNPCs.length && (
                  <div className="flex flex-col items-center justify-center p-8 gap-4 opacity-70">
                    ...
                    <p className="font-mono text-sm tracking-widest uppercase text-center mt-2">Hệ thống Lõi AI đang được kích hoạt...<br/>Đang liên kết dữ liệu vũ trụ để dệt nên sinh mệnh mới, cấu trúc thân thể, tính cách và dị năng...</p>
                  </div>
                )}
                ...
                {generatedNPCs.length > 0 && (
                  <div className="flex flex-col gap-6 w-full">
                    {generatedNPCs.map((npc, idx) => {
                      const labelClass = theme.group === 'Dark' ? 'text-cyan-400' : 'text-cyan-700';
                      return (
                        <div key={idx} className={`p-4 md:p-6 rounded-xl border shadow-inner ${theme.group === "Dark" ? "bg-white/5 border-white/10" : "bg-black/5 border-black/10"}`}>
                          <div className="flex items-center justify-between mb-4 pb-2 border-b border-black/5 dark-theme:border-white/5">
                             <div>
                                <h4 className="text-xl md:text-2xl font-bold uppercase tracking-wide">{npc.fullName || npc.name}</h4>
                                <p className="text-sm opacity-60">{npc.titles} • {npc.role}</p>
                             </div>
                             <div className="flex items-center gap-2">
                                <button
                                ...
                                >
                                   {expandedNPCs[idx] ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
                                </button>
                                <button
                          ...
                          {expandedNPCs[idx] && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 text-sm mt-4">
                               <div className="space-y-4">
                                   <div>
                                     <h5 className="font-semibold uppercase tracking-widest text-blue-500 text-xs mb-1">Cơ Bản</h5>
                                     <div className="grid grid-cols-2 gap-2 bg-black/5 dark-theme:bg-white/5 p-3 rounded-lg border border-black/5 dark-theme:border-white/5">
                                        <p><strong className={labelClass}>Nghề:</strong> {npc.occupation}</p>
                                        <p><strong className={labelClass}>Cấp bậc:</strong> {npc.rank}</p>
                                        <p><strong className={labelClass}>Giới tính:</strong> {npc.gender}</p>
                                        <p><strong className={labelClass}>Tuổi:</strong> {npc.age}</p>
                                        <p><strong className={labelClass}>Ngày sinh:</strong> {npc.dob}</p>
                                        <p><strong className={labelClass}>Chiều cao:</strong> {npc.height}</p>
                                        <p><strong className={labelClass}>Cân nặng:</strong> {npc.weight}</p>
                                        <p className="col-span-2"><strong className={labelClass}>Cơ thể:</strong> {npc.measurements}</p>
                                     </div>
                                  </div>
                                  
                                  <div>
                                     <h5 className="font-semibold uppercase tracking-widest text-purple-500 text-xs mb-1">Ngoại Hình & Lai Lịch</h5>
                                     <div className="space-y-2 opacity-80 text-justify leading-relaxed">
                                        <ExpandableText label="Biểu hiện" text={npc.appearance} theme={theme} />
                                        <p><strong className={labelClass}>Tóm tắt ngoại hình:</strong> {npc.appearanceLite}</p>
                                        <p><strong className={labelClass}>Đặc điểm nhận dạng:</strong> {npc.distinguishingFeatures}</p>
                                        <p><strong className={labelClass}>Lai lịch:</strong> {npc.background}</p>
                                     </div>
                                  </div>
                               </div>
                               ...
                               <div className="space-y-4">
                                  <div>
                                     <h5 className="font-semibold uppercase tracking-widest text-red-500 text-xs mb-1">Nội Tâm</h5>
                                     <div className="space-y-2 bg-black/5 dark-theme:bg-white/5 p-3 rounded-lg border border-black/5 dark-theme:border-white/5 leading-relaxed">
                                        <p><strong className={labelClass}>Tính cách hiển lộ:</strong> {npc.personality}</p>
                                        <p><strong className={labelClass}>Cốt lõi tính cách:</strong> {npc.personalityCore}</p>
                                        <p><strong className={labelClass}>Triết lý:</strong> {npc.philosophy}</p>
                                        <p><strong className={labelClass}>Bí mật:</strong> {npc.innerSecret}</p>
                                        <p><strong className={labelClass}>Mục tiêu:</strong> {npc.goal}</p>
                                     </div>
                                  </div>
                                  
                                  <div>
                                     <h5 className="font-semibold uppercase tracking-widest text-pink-500 text-xs mb-1">Thông Tin Mở Rộng</h5>
                                     <div className="space-y-2 bg-black/5 dark-theme:bg-white/5 p-3 rounded-lg border border-black/5 dark-theme:border-white/5 leading-relaxed">
                                        <p><strong className={labelClass}>Quan điểm tình yêu:</strong> {npc.loveViews}</p>
                                        <p><strong className={labelClass}>Kinh nghiệm tình trường:</strong> {npc.experience}</p>
                                        <p><strong className={labelClass}>Tính cách NSFW:</strong> {npc.nsfwPersonality}</p>
                                        <p><strong className={labelClass}>Phản ứng cơ thể NSFW:</strong> {npc.nsfwReactions}</p>
                                        <p><strong className={labelClass}>Sở thích, ghét, sợ (SFW):</strong> {npc.preferences?.sfw}</p>
                                        <p><strong className={labelClass}>Sở thích, ghét, sợ (NSFW):</strong> {npc.preferences?.nsfw}</p>
                                        <p><strong className={labelClass}>Chân dung văn học:</strong> {npc.literaryDescription}</p>
                                     </div>
                                  </div>

                                  {Array.isArray(npc.powers) && npc.powers.filter((p: any) => p.name || p.description).length > 0 && (
                                      <div className="space-y-2 pt-2 border-t border-black/5 dark-theme:border-white/5">
                                          <p className="text-yellow-500 font-semibold uppercase tracking-widest text-xs">Sức mạnh / Phép thuật:</p>
                                          <ul className="list-disc pl-5 space-y-1">
                                              {npc.powers.filter((p: any) => p.name || p.description).map((p: any, i: number) => (
                                                  <li key={i}>
                                                    {p.name && <strong className="text-yellow-600 dark-theme:text-yellow-400">{p.name}</strong>} 
                                                    {p.level ? ` (Cấp: ${p.level})` : ""}
                                  ...
                                  {Array.isArray(npc.skills) && npc.skills.filter((s: any) => s.name || s.description).length > 0 && (
                                      <div className="space-y-2 pt-2 border-t border-black/5 dark-theme:border-white/5">
                                          <p className="text-emerald-500 font-semibold uppercase tracking-widest text-xs">Kỹ năng:</p>
                                          <ul className="list-disc pl-5 space-y-1">
                                              {npc.skills.filter((s: any) => s.name || s.description).map((s: any, i: number) => (
                                                  <li key={i}>
                                                    {s.name && <strong className="text-emerald-600 dark-theme:text-emerald-400">{s.name}</strong>} 
                                                    {s.level ? ` (Cấp: ${s.level})` : ""}
                                  ...
                                  {Array.isArray(npc.relationships) && npc.relationships.filter((r: any) => r.name || r.description).length > 0 && (
                                      <div className="space-y-2 pt-2 border-t border-black/5 dark-theme:border-white/5">
                                          <p className="text-orange-500 font-semibold uppercase tracking-widest text-xs">Mối quan hệ:</p>
                                          <ul className="list-disc pl-5 space-y-1">
                                              {npc.relationships.filter((r: any) => r.name || r.description).map((r: any, i: number) => (
                                                  <li key={i}>
                                                    {r.name && <strong className="text-orange-600 dark-theme:text-orange-400">{r.name}</strong>}
                                                    {r.relation ? ` - ${r.relation}` : ""}
                                                    {r.status ? ` (${r.status})` : ""}
                                                    {(r.name || r.relation || r.status) && r.description ? ": " : ""}
                                                    {r.description}
                                                  </li>
                                              ))}
                                          </ul>
